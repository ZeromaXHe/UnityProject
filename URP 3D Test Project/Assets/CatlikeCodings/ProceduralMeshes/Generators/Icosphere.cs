...
namespace CatlikeCodings.ProceduralMeshes.Generators
{
    ...
    public struct Icosphere : IMeshGenerator
    {
        ...
        private static Strip CreateStrip(int id) => new()
        {
            Id = id,
        ...
        public void Execute<TS>(int i, TS streams) where TS : struct, IMeshStreams
        {
            ...
            var columnLowStart = strip.LowRightCorner + columnLowDir * ((float)u / Resolution - 1f);
            ...
            var columnTopStart = strip.HighRightCorner + columnTopDir * ((float)u / Resolution - 1f);
